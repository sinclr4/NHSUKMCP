...
{
    public class NHSOrganisationTools
    {
        private readonly AzureSearchService _searchService;
        private readonly ILogger<NHSOrganisationTools> _logger;

        public NHSOrganisationTools(ILogger<NHSOrganisationTools> logger, AzureSearchService searchService)
        {
            _logger = logger;
            _searchService = searchService;
        }

        [Function(nameof(GetOrganisationTypes))]
 
        public async Task<Dictionary<string, string>> GetOrganisationTypes(
       [McpToolTrigger(GetOrganisationsToolName, GetOrganisationsToolDescription)] ToolInvocationContext context
   )
        {
            return await Task.FromResult(OrganisationTypes.Types);
        }
        ...
        [Function(nameof(ConvertPostcodeToCoordinates))]

        public async Task<object> ConvertPostcodeToCoordinates(
             [McpToolTrigger(ConvertPostcodeToolName, ConvertPostcodeToolDescription)]
        ToolInvocationContext context,
           [McpToolProperty(LocationPropertyName, LocationPropertyDescription, true)]
        string postcode)
        {
        ...
        //SearchOrgsBy
        [Function(nameof(SearchOrgsByPostcode))]

        public async Task<object> SearchOrgsByPostcode(
     [McpToolTrigger(SearchOrgsByPostcodeToolName, SearchOrgsByPostcodeToolDescription)]
        ToolInvocationContext context,
   [McpToolProperty(LocationPropertyName, LocationPropertyDescription, true)]
        string postcode,
             [McpToolProperty(SearchOrgsTypePropertyName, SearchOrgsTypePropertyDescription, true)]
